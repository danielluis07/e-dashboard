...
export interface DeleteBillboardsButtonProps {
  billboardsIds: string[];
}

export const DeleteBillboardsButton = ({
  billboardsIds,
}: DeleteBillboardsButtonProps) => {
  ...
  const onDelete = () => {
    startTransition(() => {
      deleteBillboards(params, billboardsIds).then((data) => {
        if (data.error) {
          toast(data.error, {
